...
namespace Flames.DB
{
    public unsafe class BlockDBFile_V2 : BlockDBFile
    {
        ...
        public unsafe static int DecompressChunk(byte[] bulk, int idx, BlockDBEntry* ptr)
        {
            byte comp = bulk[idx]; 
            idx++;
            int count = bulk[idx] | (bulk[idx + 1] << 8); 
            idx += 2;

            int playerID = 0;
            if ((comp & 0x01) < 0x01) 
            { 
                playerID = ReadInt32(bulk, idx); 
                idx += 4; 
            }
            int time = 0;
            if ((comp & 0x06) < 0x06) 
            { 
                time = ReadInt32(bulk, idx); 
                idx += 4; 
            }
            int index = 0;
            if ((comp & 0x18) < 0x18) 
            { 
                index = ReadInt32(bulk, idx);
                idx += 4; 
            }
            ...
            if ((comp & 0x80) < 0x80) 
            { 
                flags = (ushort)(bulk[idx] | (bulk[idx + 1] << 8)); 
                idx += 2; 
            }
            ...
            for (int i = 0; i < count; i++)
            {
                switch (comp & 0x01)
                {
                    case 0x00: 
                        ...
                        break;
                    default: 
                        ptr->PlayerID = ReadInt32(bulk, idx); 
                        idx += 4; 
                        break;
                }
                ...
                switch ((comp & 0x06) >> 1)
                {
                    case 0: 
                        ptr->TimeDelta = time; 
                        break;
                    case 1: ptr->TimeDelta =
                            time + bulk[idx]; 
                        idx++; 
                        break;
                    case 2: 
                        ptr->TimeDelta = time + (bulk[idx] | (bulk[idx + 1] << 8)); 
                        idx += 2; 
                        break;
                    case 3: 
                        ptr->TimeDelta = ReadInt32(bulk, idx); 
                        idx += 4; 
                        break;
                }
                ...
                switch ((comp & 0x18) >> 3)
                {
                    case 0: 
                        ...
                        break;
                    case 1: 
                        ptr->Index = index + (sbyte)bulk[idx]; 
                        idx++; 
                        break;
                    case 2: 
                        ptr->Index = index + (short)(bulk[idx] | (bulk[idx + 1] << 8)); 
                        idx += 2; 
                        break;
                    case 3: 
                        ptr->Index = ReadInt32(bulk, idx); 
                        idx += 4; 
                        break;
                }
                ...
                switch (comp & 0x80)
                {
                    case 0x00: 
                        ...
                        break;
                    default: 
                        ptr->Flags = (ushort)(bulk[idx] | (bulk[idx + 1] << 8)); 
                        idx += 2;
                        break;
                }
            }
