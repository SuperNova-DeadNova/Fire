    ...
    public class CmdServer : Command
    {
        ...
        public override void Use(Player p, string message)
        {
            ...
            switch (args[0].ToLower())
            {
                ...
                case "import":
                    ...
                    break;
                case "update":
                    Find("Update").Use(p, message);
                    break;
                case "upgradeblockdb":
                    DoBlockDBUpgrade(p, args);
                    break;
                default:
                    Help(p);
                    break;
        ...
        public void DoBackup(Player p, string[] args)
        {
            ...
            if (type.CaselessEq("table"))
            {
                if (value.Length == 0)
                {
                    p.Message("You need to provide the name of the table to backup.");
                    return;
                }
                if (!Formatter.ValidName(p, value, "table")) return;
                if (!Database.TableExists(value))
                {
                    p.Message("Table \"{0}\" does not exist.", value);
                    return;
                }
        ...
        public static void DoRestore(Player p)
        {
            if (!CheckPerms(p))
            {
                p.Message("Only the Flames or the Server Owner can restore the server.");
                return;
            }
            Backup.Extract(p);
        }
        ...
        public void DoImport(Player p, string[] args)
        {
            ...
            {
                p.Message("File \"{0}\".sql does not exist.", args[1]);
                return;
            }
