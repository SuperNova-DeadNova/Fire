...
namespace Flames.Modules.Games.ZS
{
    ...
    public class QueueLevelItem : SimpleItem
    {
        ...
        public override void OnPurchase(Player p, string args)
        {
            ...
            {
                p.Message("Someone else has already queued a level."); 
                return;
            }
        ...
        public override void OnPurchase(Player p, string msg)
        {
            ...
            if (!msg.Contains("<human>") && !msg.Contains("<zombie>"))
            {
                p.Message("You need to include a \"<zombie>\" (placeholder for zombie player) " +
                               "and/or a \"<human>\" (placeholder for human player) in the infect message."); 
                return;
            }
        ...
        public override void OnPurchase(Player p, string args)
        {
            ...
            if (!ZSGame.Instance.RoundInProgress)
            {
                p.Message("You can only buy an invisiblity potion " +
                          "when a round of zombie survival is in progress."); 
                return;
            }
            ...
            if (data.Invisible) 
            { 
                p.Message("You are already invisible."); 
                return; 
            }
            ...
            {
                p.Message("You cannot buy any more invisibility potions this round."); 
                return;
            }
            ...
            {
                p.Message("You cannot buy an invisibility potion during the last minute of a round."); 
                return;
            }
    ...
    public class ReviveItem : SimpleItem
    {
        ...
        public override void OnPurchase(Player p, string args)
        {
            ...
            if (!p.infected)
            {
                p.Message("You are already a human.");
                return;
            }
            ZSConfig cfg = ZSGame.Instance.Config;
            ...
            {
                p.Message(cfg.ReviveNoTimeMessage); 
                return;
            }
            int count = ZSGame.Instance.Infected.Count;
            if (count < cfg.ReviveFewZombies)
            {
                p.Message(cfg.ReviveFewZombiesMessage); 
                return;
            }
            if (data.RevivesUsed >= cfg.ReviveTimes)
            {
                p.Message("You cannot buy any more revive potions."); 
                return;
            }
