...
namespace Flames.Modules.Games.ZS
{
    public class CmdHuman : Command
    {
        ...
        public override void Use(Player p, string message)
        {
            ...
            {
                p.Message("You cannot un-pledge that you will be infected."); 
                return;
            }
            if (ZSGame.IsInfected(p))
            {
                p.Message("You cannot use &T/human &Sas you are currently infected."); 
                return;
            }
            ...
            if (Economy.Enabled && p.money < 5)
            {
                p.Message("You need to have at least 5 &3" + Server.Config.Currency +
                                   " &Sto pledge that you will not be infected."); 
                return;
            }
            if (!ZSGame.Instance.RoundInProgress)
            {
                p.Message("Can only use &T/human &Swhen a round is in progress."); 
                return;
            }
            ...
            {
                p.Message("Cannot use &T/human &Sin last three minutes of a round."); 
                return;
            }
